...
- Workflows n8n opérationnels : veille RSS vers Google Sheet, publication Instagram automatisée (API Meta), et assistant de réponse aux emails (Gmail Trigger vers API Claude vers brouillon Gmail, avec filtrage des vrais mails clients via node IF, + threading du brouillon dans la conversation d'origine). **Testé et validé de bout en bout, threading confirmé avec une adresse externe. Premier actif démontrable en prospection.** Améliorations restantes (voir HISTORY 2026-06-23) : robustesse du parsing JSON, envoi résumé/urgence vers un canal externe (Telegram/Sheet).
