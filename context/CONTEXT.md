...
- **Manuvo** (manuvo-automaia.vercel.app) : marketplace de mise en relation particuliers / artisans (plomberie, électricité, etc.), produit AutomaIA. Application Next.js 16 déployée sur Vercel + base Neon Postgres. Modèle économique : les artisans achètent des crédits (1 crédit = 2 €) et dépensent 3 à 5 crédits pour débloquer le contact d'un particulier (max 3 artisans par demande). Socle complet et en ligne : auth (artisans + admin), packs de crédits, bacheca des demandes avec règles de déblocage, panneau admin, formulaire public sans compte, 5 langues (it défaut, en, fr, de, ar avec RTL), PWA installable, identité visuelle rouge. Fonctionnalités avancées ajoutées : matricule artisan (ART-0001) + liste admin avec contacts + export CSV, choix métiers + pays à l'inscription, mot de passe oublié par email (Resend), pages légales RGPD (titulaire affiché : AutomaIA, contact info@automa-ia.net), notifications ciblées métier+zone pour les artisans + notif de chaque demande pour l'admin (centre in-app + Web Push VAPID), et **paiement réel des crédits par Stripe Checkout** (validé en mode test). **Emails transactionnels en ligne** : domaine automa-ia.net vérifié dans Resend (DKIM + SPF via DNS Wix), expéditeur no-reply@automa-ia.net, envoi réel vers n'importe quel destinataire (réinitialisation de mot de passe fonctionnelle en production). **Web Push opérationnel** (testé sur iPhone en PWA : notification + son), avec clés VAPID régénérées et cohérentes dans Vercel. Retouches récentes : bacheca artisan filtrée par ses métiers uniquement (partout : liste, cloche), colonne Métiers dans la liste admin + export CSV, et **validation stricte du téléphone du particulier** (via libphonenumber-js selon le pays, numéro normalisé au format international) pour éviter qu'un artisan paie des crédits pour un contact inutilisable. Comptes de démo : admin@manuvo.app / artigiano@manuvo.app. À finaliser côté Hatem : la P.IVA dans les pages légales quand elle existera, et passer Stripe en mode Live (vérification compte + IBAN, clés live + webhook live) avant les vrais paiements.
